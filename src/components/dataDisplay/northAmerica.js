...
function NorthAmerica({ continents }) {

    const [countries, setCountries] = useState(countriesData.countries);
    const [continent, setContinent] = useState(null);

    useEffect(() => {
        let data = countries.filter(obj => obj.continent === 5).sort((a, b) => a.name.localeCompare(b.name));
        setCountries(data);
        let continentData = continents.filter(obj => obj.id === 5);
        setContinent(continentData[0]);
    }, []);

    return (
        <Card title="Countries in North America">
