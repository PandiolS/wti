...
function Africa({ continents }) {

    const [countries, setCountries] = useState(countriesData.countries);
    const [continent, setContinent] = useState(null);

    useEffect(() => {
        let data = countries.filter(obj => obj.continent === 3).sort((a, b) => a.name.localeCompare(b.name));
        setCountries(data);
        let continentData = continents.filter(obj => obj.id === 3);
        setContinent(continentData[0]);
    }, []);

    return (
        <Card title="Countries in Africa">
